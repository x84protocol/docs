...
<Steps>
  <Step title="Creator builds agent">
    Use the [agent builder](/hosting/agent-builder) wizard or visual graph editor to define the agent's identity, system prompt, LLM, MCP tools, skills, and pricing.
  </Step>
  ...
  <Step title="A2A endpoint live">
    The [A2A gateway](/hosting/a2a-gateway) starts serving the agent at `https://a2a.x84.dev/agents/{nft-mint}/`. The agent is discoverable in the marketplace.
  </Step>
...
<Columns cols={2}>
  ...
  <Card title="Agent Builder" icon="wand-magic-sparkles" href="/hosting/agent-builder">
